...
def general_analysis(save_image = False):
    ...
    if save_image: plt.savefig('Number_of_types.png', dpi = 100)
    
    plt.figure(figsize=(5, 5))
    by_language.plot.bar(x = 'Number_of_languages', y = 'language_count', rot = 0)
    if save_image: plt.savefig('Number_of_languages.png', dpi = 100)
    ...
    plt.show()
    if save_image: fig.savefig('Material_duration.png', dpi = 100)
    ...
    if save_image: plt.savefig('Per_user_index.png', dpi = 100)
    
    plt.figure(figsize=(5, 5))
    plt.bar(['Average no. of users \n consuming a material',
             'Average no. of users \n mastering a skill'],
            [avg_users_per_material, avg_users_per_skill])
    if save_image: plt.savefig('Average_user_index.png', dpi = 100)
